...
export function requestState(request: any) {
  if (!request) {
    return 0;
  }
  if (!request.response) {
...
export function canEditRequest(request: any) {
  return requestState(request) < 2;
}

export function canEditResponse(request: any) {
  return requestState(request) === 3;
}

export function prettyUrl(data : FlowType, query: boolean = false) {
