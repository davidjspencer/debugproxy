...
import { updateRequest, getRequest} from 'lib/api';
...
export function updateRequestStateComplete() {
  return {
    type: "REQUEST_UPDATE_STATE_COMPLETE"
  }
}

export function updateRequestState(data: any) {
  return {
...
export function updateRequestHeader() {
  return (dispatch: RequestDispatchType, getState: () => StateType) => {
    var state = getState();
...
export function quickDeleteRequestHeader(index: number) {
  console.log(index)
  return (dispatch: RequestDispatchType, getState: () => StateType) => {
    var state = getState();
